 ...
 * the benchmark that measures them. Footer-linked (see App.tsx's `#glassbox`
 * route), not part of the primary tab nav: this is a reference page for
 * evaluators, not a feature a photographer needs day to day.
 *
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted">
            Fetched straight from the running API — this is whatever user is currently scoped
            (demo-user in judge mode).
          </p>
        </div>
        <button
          type="button"
...
const BenchmarkSummaryStrip: React.FC = () => {
  const { mean_fama: meanFama } = defaultResults.summary;
  ...
  return (
    <p className="text-sm text-stone-300 leading-relaxed">
      ...
      (without) · {formatRatio(defaultResults.summary.mean_token_savings_ratio)} fewer context tokens · recall
      accuracy identical in both configs
      ...
      <div className="max-h-80 overflow-y-auto overflow-x-auto">
        <table className="w-full text-xs">
          <thead className="sticky top-0 bg-surface-2 text-muted uppercase tracking-wide text-[10px]">
            <tr>
              <th className="text-left font-semibold px-3 py-2">Trace</th>
              <th className="text-right font-semibold px-3 py-2">FAMA default</th>
              <th className="text-right font-semibold px-3 py-2">FAMA ablated</th>
              <th className="text-right font-semibold px-3 py-2">Token savings</th>
            </tr>
          </thead>
          <tbody>
            {defaultResults.results.map((row) => {
              ...
              return (
                <tr key={row.user_id} className="border-t border-warm/50 odd:bg-surface-1 even:bg-transparent">
                  <td className="px-3 py-1.5 text-stone-300 font-mono">{row.user_id}</td>
...
const WorkedExample: React.FC = () => {
  ...
  return (
    <div className="rounded-xl border border-brand-500/30 bg-brand-500/5 p-4 space-y-3">
      <Eyebrow tone="brand">Worked example — {WORKED_TRACE_ID}</Eyebrow>
      <p className="font-serif text-base text-white">Q: What camera gear do I use?</p>
      <div className="space-y-2 text-sm">
        <p className="text-stone-300">
          <span className="font-semibold text-stone-200">Without forgetting, Engram would recall:</span>{' '}
          {workedNoForgetting.recalled_contents.join('; ')}
        </p>
        <p className="text-stone-300">
          <span className="font-semibold text-brand-400">With forgetting:</span>{' '}
          {workedDefault.recalled_contents.join('; ')}
        </p>
      </div>
