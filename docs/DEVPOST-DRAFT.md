...
- Every non-obvious call along the way — port vs. rebuild, keeping MongoDB Atlas, shipping a real MCP server, deploying around a stuck verification queue, same-origin frontend serving, moving chat off the reasoning tier, HTTPS via Caddy, SSE streaming — is written up as an ADR in [`docs/architecture/`](https://github.com/prasadt1/engram/tree/main/docs/architecture): context, the alternatives I didn't take, and why.
...
- ✅ (Jul 4) **The shape-repair armor fired in production on day one.** During the first full HTTP end-to-end test, Qwen-VL drifted from the JSON contract once more (1 validation error) — and the repair retry caught it, fixed it, and the request completed with a clean 200. The failure mode that would have been a raw 500 in front of judges became an invisible recovery in a log line. Building the armor *before* needing it is the whole point.
- ✅ (Jul 4) **A mentor reply that took 78 seconds — and a mocked test that couldn't have caught it.** A user report of chat timeouts led me to time the actual call: mentor chat was routed to the reasoning-tier model (`qwen3.7-max`) for a plain conversational reply — 77.8s, live, measured. Switched it to the fast tier already proven by Reflection; latency dropped to 6-28s. Then I added SSE streaming on top for perceived responsiveness, and found a second bug the same way: DashScope emits a trailing usage-only chunk with an empty `choices` list, which crashed my streaming generator with an unhandled `IndexError` and silently truncated every reply. My unit tests, which mock the SDK client, never produced that chunk shape — only a live call against the real API surfaced it. Lesson, twice in one afternoon: mocked coverage proves your code handles what you imagined the API would send, not what it actually sends.
- ✅ (Jul 4) **The prompt section that talked about you behind your back.** First live chat replies came out as third-person bullet notes — "Guide the hobbyist toward off-center framing" — like a coach writing case files instead of talking to you. Cause: a vestigial "Glass Box" section inherited from Iris's prompt, where it fed a UI panel; in Engram, transparency lives in the Memory Receipt, so the section just leaked report-format into chat. Removed it, added one line enforcing second person, re-tested live: "Thinking about your recent still life work, I'm looking back at that shot of the terracotta pot…" Night and day, zero code changed.
...
- **Architecture Decision Records:** ✅ `docs/architecture/` — 8 ADRs covering every non-obvious engineering call (port-vs-rebuild, MCP-vs-REST, PAYG deployment around a stuck verification queue, model-tier latency fix, HTTPS, SSE streaming), each with the alternatives considered and why they were rejected. Worth a judge's click if evaluating Technical Depth.
