...
3. **Library (existing grid, memory overlay).** Genre filter chips, milestone badges on photos ("first cleared horizon," "best night shot"), "biggest improvement" sort. Photo URLs switch from GCS to **Alibaba OSS signed URLs** — invisible to the grid code. Clicking a photo opens the **photo detail split view** (below), replacing the full-screen `ImageLightbox` modal for this flow.
4. **Photo detail split view (new page/route, not a modal).** Two-pane layout: left pane = the photo plus its critique, genre tag, and any milestone badge; right pane = the photo-scoped **MentorChat**. Decided over extending `ImageLightbox.tsx` because the existing component is a full-screen dark modal with little room for a chat panel and cramped mobile behavior — a dedicated split view demos better and stacks cleanly on small screens (photo on top, chat below). `ImageLightbox` itself is untouched and can stay in use elsewhere (e.g. a lightweight zoom-to-fullscreen action from within this view).
5. **MentorChat (one reusable component, two modes).** Extracted from `MentorTab`/`MentorChatTurn`. Props: `scope: 'global' | { photoId }`. Global mode = Mentor page; scoped mode = the right pane of the photo detail split view. Backend: the existing chat route `POST /api/v1/agent/chat` (`app/api/server.py:199`) gains optional `photo_id`; `recall()` filters memory to that photo + related history. Standard response shape baked into the prompt: *recall specifics → celebrate retired items → current focus.* A collapsible glass-box panel under each reply shows the actual `recall()` call and memories used.
6. **Glass box / benchmark (new, small, judge-facing).** Memory stats (`get_memory_stats()`), benchmark results table, token-savings chart. Footer-linked; ignorable by normal users.
