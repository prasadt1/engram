...
python -m eval.run --compare                                         # side-by-side table from committed JSONs
```
...
Pulled directly from the committed `eval/results-default.json` and
`eval/results-no-forgetting.json` (`recalled_contents` field, `trace_1`):
...
Generated by `python -m eval.run --compare`, reading the two committed
JSONs (`eval/results-default.json`, `eval/results-no-forgetting.json`).
Re-run the two commands at the top of this file to regenerate those JSONs
from scratch; `--compare` itself never re-runs the engine — it only reads
and formats what's already committed on disk. Re-running default twice in
a row produces byte-identical JSON (verified at freeze time — no wall
clock in scoring, timestamps synthesized from session indices against a
fixed anchor).

| trace | FAMA default | FAMA no-forgetting | Δ FAMA | tokens default | tokens no-forgetting | savings (default) |
|---|---|---|---|---|---|---|
| trace_1 | 1.0 | 0.5 | 0.5 | 8 | 16 | 2.0x |
| trace_2 | 1.0 | 0.5 | 0.5 | 15 | 27 | 1.8x |
| trace_3 | 1.0 | 1.0 | 0.0 | 21 | 21 | 1.0x |
| trace_adversarial_1 | 1.0 | 0.5 | 0.5 | 10 | 19 | 1.9x |
| trace_adversarial_2 | 1.0 | 0.5 | 0.5 | 11 | 23 | 2.09x |
| trace_adversarial_3 | 1.0 | 0.5 | 0.5 | 12 | 22 | 1.83x |
| trace_adversarial_4 | 1.0 | 0.5 | 0.5 | 13 | 22 | 1.69x |
| trace_adversarial_5 | 1.0 | 0.5 | 0.5 | 12 | 26 | 2.17x |
| trace_adversarial_6 | 1.0 | 0.5 | 0.5 | 17 | 29 | 1.71x |
| trace_4 | 1.0 | 0.5 | 0.5 | 13 | 24 | 1.85x |
| trace_5 | 1.0 | 0.5 | 0.5 | 11 | 26 | 2.36x |
| trace_6 | 1.0 | 0.5 | 0.5 | 15 | 27 | 1.8x |
| trace_7 | 1.0 | 1.0 | 0.0 | 15 | 15 | 1.0x |
| trace_8 | 1.0 | 0.5 | 0.5 | 16 | 30 | 1.88x |
| trace_9 | 1.0 | 1.0 | 0.0 | 17 | 17 | 1.0x |
| trace_10 | 1.0 | 0.5 | 0.5 | 15 | 30 | 2.0x |
| trace_11 | 1.0 | 1.0 | 0.0 | 18 | 18 | 1.0x |
| trace_12 | 1.0 | 0.5 | 0.5 | 16 | 31 | 1.94x |
| trace_13 | 1.0 | 1.0 | 0.0 | 15 | 15 | 1.0x |
| trace_chain_1 | 1.0 | 0.3333 | 0.6667 | 13 | 39 | 3.0x |
| trace_chain_2 | 1.0 | 0.3333 | 0.6667 | 14 | 34 | 2.43x |
| trace_chain_3 | 1.0 | 0.3333 | 0.6667 | 13 | 36 | 2.77x |
| trace_multihop_1 | 1.0 | 0.6 | 0.4 | 50 | 74 | 1.48x |
| trace_multihop_2 | 1.0 | 1.0 | 0.0 | 47 | 47 | 1.0x |
| trace_multihop_3 | 1.0 | 1.0 | 0.0 | 54 | 54 | 1.0x |
| trace_multihop_4 | 1.0 | 1.0 | 0.0 | 27 | 27 | 1.0x |
...
**FAMA gap (default − no-forgetting): 0.3615** — the no-forgetting ablation
lets superseded facts leak back into recall, which is exactly what FAMA's
FAA term penalizes. Recall accuracy (MPA) is identical in both configs —
every trace hits 100% of `expects_current` regardless of ablation — so the
engine is not trading recall for forgetting; it gets both at ~1.72x lower
token cost than full-history stuffing. (This summary block is the one
Task 32 lifts into the root `README.md`.)
